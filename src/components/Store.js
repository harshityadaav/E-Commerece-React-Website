  ...
  return (
      <div id='store'>
          {clothes.map((clothes) => {
            return (
                <div id='card'>
                  <div id='image'><img style={{width:"300px" , height:"200px"}} src={clothes.url} alt="" /></div>
                  <div id="box">                 
                      <div><b style={{fontStyle:"italic"}} > {clothes.name} </b></div>
                      <div><b style={{fontStyle:"italic"}}>$ {clothes.price}.00</b></div>
                      <div><Form id={clothes.id} name={clothes.name} price={clothes.price} url={clothes.url} cart={props.cart} handleClick={props.handleClick} handleChange={props.handleChange}  /></div>
                  </div>
                </div>
            );
          })}
